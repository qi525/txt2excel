...
# 定义历史记录文件路径
history_xlsx_path = os.path.join(python_history_folder, 'scan_history.xlsx')
...
def detect_types(line, cleaned):
    ...
    # monochrome类型
    if 'monochrome' in lower_line: # 新增：检测monochrome
        types.append('黑白原图')
    return ','.join(types)
...
def clean_tags(line):
    ...
    # 新增：去掉含有monochrome的tag
    cleaned_tags = [tag for tag in cleaned_tags if 'monochrome' not in tag.lower()]
    # 检查是否含有敏感词
    has_sensitive = any(
...
# 写入历史记录
# ====================================================================
def update_history(history_file_path, folder_path, total_scanned, found_count, not_found_count):
    try:
...
# 调用更新历史记录函数
update_history(history_xlsx_path, folder_path, total_files_scanned, found_txt_count, not_found_txt_count)
