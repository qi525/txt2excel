...
# 之前的导入可能是：
# from my_logger import LogManager, normalize_drive_letter # 如果你还在混合使用

# 现在你需要明确导入 setup_logger 函数
from my_logger import setup_logger # 导入 setup_logger 函数
from loguru import logger           # 导入 Loguru 的 logger 对象
# from my_logger import normalize_drive_letter # normalize_drive_letter 现在应该在my_logger.py中，可以这样导入

my_logger.setup_logger(log_output_dir)
    ...
    try:
        ...
        for file_path_to_open in files_to_open:
            if not file_path_to_open.exists():
                main_logger.level(f"警告: 尝试打开不存在的文件: {normalize_drive_letter(str(file_path_to_open))}", level="WARNING")
                continue
            ...
            if not is_allowed_to_open:
                main_logger.level(f"拒绝自动打开没有时间戳或不符合命名约定的源文件: {normalize_drive_letter(str(file_path_to_open))}", level="WARNING")
            ...
            main_logger.level(f"自动打开: {normalize_drive_letter(str(file_path_to_open))}", level="INFO")

    except Exception as e:
        main_logger.level(f"错误: 自动打开文件失败. 错误: {e}", level="ERROR")
...
def main():
    ...
    # error_warning_logger.level("错误和警告日志文件已创建。", level="INFO", to_error_log=False) 
    ...
    main_logger.level(f"程序启动. 脚本目录: {normalize_drive_letter(str(script_dir))}", level="INFO")
    main_logger.level(f"错误和警告日志将写入到: {normalize_drive_letter(str(error_warning_log_manager.log_file_path))} (仅当出现警告或错误时创建)", level="INFO") # 修改提示信息
    
    if not create_directory_if_not_exists(history_folder_path, main_log_manager):
        main_logger.level("致命错误: 无法创建历史记录文件夹，程序退出。", level="CRITICAL")
    ...
    if not create_directory_if_not_exists(cache_folder_path, main_log_manager):
        main_logger.level("致命错误: 无法创建缓存文件夹，程序退出。", level="CRITICAL")
        error_warning_log_manager.close() # 确保关闭错误日志
        sys.exit(1)
    
    # 新增：确保反推记录文件夹存在
    if not create_directory_if_not_exists(output_base_dir, main_log_manager):
        main_logger.level("致命错误: 无法创建输出文件夹 (反推记录)，程序退出。", level="CRITICAL")
    ...
    main_logger.level(f"在 '{normalize_drive_letter(str(batch_file_path))}' 中检测到 {len(folders_to_scan)} 条有效地址。", level="INFO")

    if not folders_to_scan:
        main_logger.level("没有找到要扫描的文件夹路径，程序终止。", level="INFO")
    ...
    for folder_path in folders_to_scan:
        ...
        scan_logger.level(f"开始扫描 {normalize_drive_letter(str(folder_path))}", level="INFO")
        ...
        try:
            ...
            for attempt in range(MAX_SAVE_RETRIES):
                try:
                    ...
                    scan_logger.level(f"扫描结果已保存到: {normalize_drive_letter(str(current_excel_file))} (尝试 {attempt + 1}/{MAX_SAVE_RETRIES})", level="INFO")
            ...
            if not save_successful: # 如果所有重试都失败了
                ...
                try:
                    ...
                    scan_logger.level(f"成功将扫描结果保存到备用位置: {normalize_drive_letter(str(fallback_excel_file))}", level="WARNING")
            ...
            main_logger.level(f"本次扫描历史记录已成功添加至内存。", level="INFO")
            ...
            main_logger.level(f"处理文件夹 {normalize_drive_letter(str(folder_path))} 时发生错误: {e}", level="ERROR")
            print(f"处理文件夹 {folder_path} 时发生错误。错误: {e}")
        finally:
            scan_log_manager.close() 
            main_logger.level(f"--- 完成处理文件夹: {normalize_drive_letter(str(folder_path))} ---\n", level="INFO")
            print(f"--- 完成处理文件夹: {folder_path} ---\n")

    # 所有文件夹处理完毕后，将内存中的历史记录保存到最终的Excel文件
    main_logger.level(f"所有扫描任务完成，开始将历史记录保存到最终的Excel文件: {normalize_drive_letter(str(final_history_excel_path))}", level="INFO")
    main_logger.level(f"准备保存 {len(history_manager.history_data)} 条历史记录到Excel。", level="INFO") # 确认即将保存的数量
    main_logger.level(f"最初在 '{normalize_drive_letter(str(batch_file_path))}' 中检测到 {len(folders_to_scan)} 条有效地址。", level="INFO") # 再次提示最初检测到的有效地址数量
    ...
    if save_history_success:
        main_logger.level("历史记录已成功保存到Excel。", level="INFO")
        ...
        if create_directory_if_not_exists(cache_folder_path, main_log_manager):
            ...
            main_logger.level(f"开始复制历史记录到缓存文件夹: {normalize_drive_letter(str(cached_history_file_path))}", level="INFO")
            copy_cache_success = copy_file(final_history_excel_path, cached_history_file_path, main_log_manager)
            
            if copy_cache_success:
                main_logger.level("历史记录已成功复制到缓存文件夹。", level="INFO")
                final_files_to_open_at_end.append(cached_history_file_path) # 如果复制成功，则打开缓存文件
            else:
                main_logger.level("历史记录复制到缓存文件夹失败。", level="ERROR")
        else:
            main_logger.level(f"无法创建缓存文件夹: {normalize_drive_letter(str(cache_folder_path))}，将无法复制历史记录。", level="ERROR")
            
    else:
        main_logger.level("历史记录保存到Excel失败，将不会自动打开历史Excel文件。", level="ERROR")
    ...
    if error_warning_log_manager.log_file_path and error_warning_log_manager.log_file_path.exists():
        ...
        main_logger.level("警告: 错误和警告日志文件不存在或路径无效，无法自动打开。", level="WARNING")

    # 最终的自动打开操作
    open_output_files_automatically(final_files_to_open_at_end, main_log_manager)

    main_logger.level("所有文件夹处理完毕，程序即将退出。", level="INFO")
